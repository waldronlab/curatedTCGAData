...
`curatedTCGAData` now has a version `2.0.0` set of data with a number of
...
Here is a list of changes to the data provided in version `2.0.0`
...
```{r,eval=FALSE}
curatedTCGAData(
    diseaseCode = "*", assays = "*", version = "2.0.0"
)
```
...
head(
    curatedTCGAData(
        diseaseCode = "COAD", assays = "*", version = "2.0.0"
    )
)
```
...
(accmae <- curatedTCGAData(
    "ACC", c("CN*", "Mutation"), version = "2.0.0", dry.run = FALSE
))
```
